...
(use-package dashboard
  ...
  :config
  (require 'seq)
  (defun aic/dashboard-open-when-empty ()
    "Open the dashboard only when Emacs starts without visiting files."
    (when (and (not (daemonp))
               (not (seq-some #'buffer-file-name (buffer-list))))
      (dashboard-open)))
  (add-hook 'emacs-startup-hook #'aic/dashboard-open-when-empty))
...
(use-package markdown-mode
  :mode (("README\\.md\\'" . gfm-mode)
         ("\\.md\\'" . markdown-mode))
  :init
  ;; Use Pandoc for robust conversion (supports MathJax, etc.).
...
(use-package markdown-preview-mode
  :commands (markdown-preview-mode markdown-preview-toggle)
  :bind (:map markdown-mode-command-map
              ("x" . markdown-preview-mode)))

(use-package markdown-dnd-images
  :ensure nil
  :if (locate-library "markdown-dnd-images")
  :after markdown-mode
  :config
...
(defun aic/markdown-paste-image ()
  "Paste an image from the clipboard into ./images and insert link."
  (interactive)
  (let* ((img-dir "images")
         (img-name (format-time-string "img_%Y%m%d_%H%M%S.png"))
         (rel-path (concat "./" img-dir "/" img-name))
    ...
    (let ((cmd
           (cond
            ((eq system-type 'darwin)
             (format "pngpaste %s" (shell-quote-argument abs-path)))
            ((eq system-type 'gnu/linux)
             (format "xclip -selection clipboard -t image/png -o > %s"
                     (shell-quote-argument abs-path)))
            ((eq system-type 'windows-nt)
             (format "powershell -command \"(Get-Clipboard -Format image).Save('%s')\""
                     abs-path))))))
      (when (and cmd (zerop (shell-command cmd)))
        (insert (format "![image](%s)" rel-path))
        (when (fboundp 'markdown-toggle-inline-images)
          (markdown-toggle-inline-images)))))

(with-eval-after-load 'markdown-mode
  (define-key markdown-mode-map (kbd "C-c C-v") #'aic/markdown-paste-image))
